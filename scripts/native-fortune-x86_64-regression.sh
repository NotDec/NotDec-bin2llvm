...
usage() {
  cat <<'EOF'
usage: native-fortune-x86_64-regression.sh NOTDEC_NATIVE_LLVM LLVM_BIN SOURCE_DIR BUILD_DIR
EOF
}
...
OUT_DIR="${NOTDEC_NATIVE_FORTUNE_X86_64_OUT_DIR:-$BUILD_DIR/native-fortune-x86_64-regression}"
PROTO_JSON="$SOURCE_DIR/tests/fixtures/native/fortune-x86_64.external-prototypes.json"
...
set +e
FORTUNE="$("$FETCH_FIXTURE" fortune-x86_64 --source-dir "$SOURCE_DIR")"
FETCH_RC=$?
set -e

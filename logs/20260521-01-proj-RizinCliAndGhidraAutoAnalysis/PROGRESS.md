...
- [ ] 阶段 3: Recursive disassembly CFG
  ...
  - [x] 已补初始 seed 按 confidence 分层入队，避免低可信 seed 抢占高可信入口预算。
  ...
  - [x] 已补 Low confidence seed 不参与函数边界截断，只作为候选入口。
...
- [ ] 阶段 4: CLI query
  ...
  - [x] 已补 `notdec-native-discover --function-xrefs-json` 按 confirmed function 入口输出单函数引用。
  ...
  - [x] 已补 `notdec-native-discover --block-json` 按 confirmed basic block 起点查单 block 和指令。
  ...
  - [x] 已补 `notdec-native-discover --xrefs-dot` 输出全量 xref DOT。
